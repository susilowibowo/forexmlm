...
class DataRowsTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $memberDataType = DataType::where('slug', 'members')->firstOrFail();
        ...
        $dataRow = $this->dataRow($memberDataType, 'name');
        if (!$dataRow->exists) {
            $dataRow->fill([
                'type'         => 'text',
                'display_name' => 'Inisial Member',
                'required'     => 1,
                'browse'       => 1,
                'read'         => 1,
                'edit'         => 1,
                ...
                'details'      => json_encode([
                    'validation' => [
                        'rule' => 'required|string|max:150|regex:/^[a-zA-Z0-9\s]+$/'
                        ],
                ]),
                'order'        => 1,
            ])->save();
        ...
        $dataRow = $this->dataRow($memberDataType, 'parent_id');
        if (!$dataRow->exists) {
            $dataRow->fill([
